...
The henrulle framework allows you to run through a sequence of functions the number of times specified by the `attempts` parameter (default 2). The framework encourages functional programming conventions: exceptions are suppressed and do not affect the flow, and the functions in sequence should be pure and should not change local state or act upon state changed by another function. An exception to this rule is that a function can always stop the whole sequence by returning `True`.
